<?php
	...
	class SiteUsersModel extends CRUDModel {
		...
		public function __construct() {
			...
			$this->phoneNumber = new ModelTextField("phoneNumber", $messages['field_phone'], true, false);
			$this->phoneNumber->required = false;
			...
			$this->lpwToken = new ModelTextField("lpwToken", '', true, false);
			$this->lpwToken->hidden = true;
			$this->lpwToken->required = false;
			
			$this->authToken = new ModelTextField("authToken", '', true, false);
			$this->authToken->hidden = true;
			$this->authToken->required = false;
		}
		...
		public static function getByAuthToken($token) {
			return self::find(" WHERE `authToken` = '{#1}'", Array($token));
		}
