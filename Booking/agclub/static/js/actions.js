...
// select-controller.js end

// horizontal-slider-controller.js
...
function HorizontalSliderController(identifier) {
	...
	function onResize() {
		...
		var margin = 0;
		if (parseInt(sliderItem.css('margin-left'))) margin += parseInt(sliderItem.css('margin-left'));

		if (parseInt(sliderItem.css('margin-right'))) margin += parseInt(sliderItem.css('margin-right'));
		if (parseInt(sliderItem.css('padding-left'))) margin += parseInt(sliderItem.css('padding-left'));
		if (parseInt(sliderItem.css('padding-right'))) margin += parseInt(sliderItem.css('padding-right'));

		var sliderItemWidth = parseInt(sliderItem.width()) + margin;
...
// horizontal-slider-controller.js end

// m-slider-controller.js

var allowSlideNext = true;

function MSliderController(identifier) {

	var self, slider = {}, index = 0;

	init();

	function init() {
		self = $(identifier);
		slider.items = self.find('.mslider-item');
		slider.slidesCount = slider.items.length;

		var segments = tmpl($('#mslider-segment-template').html(), {slidesCount : slider.slidesCount});
		self.find('.mslider-top').prepend(segments);

		DragMaster.onDrag(function(){}).makeDraggable(self.find('.mslider-pointer'));

		setInterval(function(){
			if (allowSlideNext) slideNext(index);
		}, 7000);

	}

	function slideTo() {
		var itemWidth = parseInt(slider.items.width());
		self.find('.mslider-ribbon').animate({
			'left': -(index * itemWidth) + 'px',
		});
		$('.mslider-pointer').animate({
			'left': parseInt(index * parseInt($('.mslider-segment').width())) + 'px',
		});
	}

	function slideNext() {
		if (index < slider.slidesCount - 1) {
			index++;
		}
		else {
			index = 0;
...
var App = {
	...
	init: function() {
		...
		$('.show-login-container').unbind('click').click(this.showLoginContainer);
		$('.show-reg-container').unbind('click').click(this.showRegContainer);
	...
	showLoginContainer: function() {
		$(this).parents('.auth-all-container').find('.show-login-container').hide();
		$(this).parents('.auth-all-container').find('.registration-block-container').fadeOut('fast',function(){
			$(this).parents('.auth-all-container').find('.login-container').fadeIn();
			$(this).parents('.auth-all-container').find('.show-reg-container').show();
			$(this).parents('.auth-all-container').find('.lpw-container').hide();
		});
	},
	showRegContainer: function() {
		$(this).parents('.auth-all-container').find('.show-reg-container').hide();
		$(this).parents('.auth-all-container').find('.login-container').fadeOut('fast', function(){
			$(this).parents('.auth-all-container').find('.registration-block-container').fadeIn();
			$(this).parents('.auth-all-container').find('.show-login-container').show();
			$(this).parents('.auth-all-container').find('.lpw-container').hide();
		});
	},
	submitRegistration: function() {
		var email = $(this).parents('.auth-all-container').find('.reg-email').val(),
		csrfKey = $('#csrf-key').val();
		var messageContainer = $(this).parents('.auth-all-container').find('.registration-errors');
	...
	submitLogin: function() {
		var email = $(this).parents('.auth-all-container').find('.login-email-input').val(),
		password = $(this).parents('.auth-all-container').find('.login-password-input').val(),
		csrfKey = $('#csrf-key').val();
		$.ajax({
	...
	callTo: function() {
		...
		$.ajax({
			...
			url: url,
			dataType: 'json',
			data: 'name=' + name + '&number=' + phone + '&time=' + time + '&subject=' + subject + '&csrf_key=' + csrfKey + '&captcha_keystring=' + $('#call-to-bank-captcha').val(),
...
function lostPsw() {
	var mail = $(this).parents('.auth-all-container').find('.user-mail-lpw').val();
	if (mail.length) {
		$.ajax({
...
function lostPswSlide() {
	$(this).parents('.auth-all-container').find('.login-container').slideUp();
	$(this).parents('.auth-all-container').find('.lpw-container').slideDown();
	$(this).parents('.auth-all-container').find('.show-login-container').show();
	$(this).parents('.auth-all-container').find('.show-reg-container').show();
}

function slideRegIn1ColumnMode() {
	$('.reg-login-container-479').slideToggle();
}
...
function reloadCallToCaptcha() {
	$('#call-to-captcha-image').attr('src', $('#call-to-captcha-image').attr('src') + '&1');
}
