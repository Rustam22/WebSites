...
{block name="content"}
	<div class="view-content-container">
		<div class="view-content-title-container width100">
			...
			<div class="left view-content-title text-color-blue font16"><span>{$messages.news.title}</span></div>
			<div class="clear"></div>
		</div>
		...
		<div class="news-list-item-container width100">
			...
			<div class="news-list-item-title">
				<a class="font12 bold text-color-blue" href="{$app_url}/{$currentLang}/news/view/{$n.r_id}/{$n.itemTitle|urlencode}">{$n.itemTitle}</a>
			</div>
			<div class="news-list-item-description text-color-gray font14">
				{$n.description} <span class="text-color-blue font14">[</span><a href="{$app_url}/{$currentLang}/news/view/{$n.r_id}/{$n.itemTitle|urlencode}" class="news-more text-color-blue">...</a><span class="text-color-blue font14">]</span>
			</div>
		</div>
		{/foreach}
			...
			{foreach from=$paginator item=p}
				...
				{else}
					<div class="paginator-item left bg-color-gray-dark">
						...
						{else}
							<a href="{$app_url}/{$currentLang}/news/page/{$p.key}" class="font14 text-color-blue" >{$p.title}</a>
						{/if}
					</div>
				{/if}
			{/foreach}
