...
{block name="content"}
	...
	<div class="view-page-content view-page-content-small font16">
		<div class="page-title news-page-title font25">{$messages.common.companies}</div>
		...
		{foreach from=$newsArchive item=n}
			<div class="news-archive-item">
				...
				<div class="item-description font13">
					{$n.description}
					<a href="{$app_url}/{$currentLang}/companies/view/{$n.r_id}" class="item-more font13">{$messages.news.more}</a>
				</div>
				...
				{foreach from=$paginator item=p}
					...
					{else}
						<div class="paginator-item">
							...
							{else}
								<a href="{$app_url}/{$currentLang}/companies/page/{$p.key}" class="font13" >{$p.title}</a>
							{/if}
						</div>
					{/if}
				{/foreach}
