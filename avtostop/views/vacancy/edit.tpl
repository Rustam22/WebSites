...
{block name="content"}
	...
	<div class="vacancy-prof-area">
		...
		<div class="choose-specn" id="choose-specialization-container">
			...
			<div class="left prof-area-choosed"><span>{$profArea[$vacancy['profArea']]}</span></div>
			<div class="left"><input type="button" class="blue-button-input" id="choose-spec" value="{$m.choose}" /></div>
			<div class="clear"></div>
			<input type="hidden" name="profArea" id="prof-area-value" value="{$vacancy['profArea']}" />
		</div>
		...
		<div class="vac-left-side left">
			<div class="vac-city-container">
				<div id="vac-country" >
					<span class="c-pointer value-element tx-green">{$vacancy['cityTitle']}</span>
					<input type="hidden" name="cityId" id="vac_city_id" class="select-input" value="{$vacancy['cityId']}" />
					<input type="hidden" name="countryId" id="vac_country_id" class="select-input"  value="{$vacancy['countryId']}" />
				</div>
				<script>
			...
			<div class="vac-salary-container">
				<div class="salary-from left">
					<input type="text" name="salaryFrom" value="{$vacancy['salaryFrom']}" />
				</div>
				<div class="salary-splitter left">-</div>
				<div class="salary-to left">
					<input type="text" name="salaryTo" value="{$vacancy['salaryTo']}" />
				</div>
				<div class="salary-currency left">
					<div id="select-currency" class="select-container p-relative" sel-val="{$vacancy['salaryCurrency']}" >
								...
								{foreach from=$experiences item=v key=k}
									<div class="afc-radio-container radio-button-item-container" data-value="{$k}" >
										<div class="div left radio-button-own">
											{if ($k == $vacancy['experience'])}
											<div class="sprite radio-sprite radio-checked-sprite"><input type="hidden" name="experience" value="{$k}" /></div>
											{else}
						...
						<div class="afc-field-input afc-field-own">
							<div class="radio-button-container" id="radio-work-time" data-name="workTime" >
								{foreach from=$workTimes item=v key=k}
									<div class="afc-radio-container radio-button-item-container" data-value="{$k}" >
										<div class="div left radio-button-own">
											{if ($k == $vacancy['workTime'])}
											<div class="sprite radio-sprite radio-checked-sprite"><input type="hidden" name="workTime" value="{$k}" /></div>
											{else}
											<div class="sprite radio-sprite"></div>
											{/if}
										</div>
		...
		<div class="vac-right-side left">
			<div class="vac-contact-user">
				...
				<div class="vcu-input"><input type="text" class="vac-input" name="contactUser" value="{$vacancy['contactUser']}" /></div>
			</div>
					...
					<div class="afc-field-input afc-field-own">
						<div class="radio-button-container" id="radio-busy-time" data-name="busyTime" >
							{foreach from=$busyTimes item=v key=k}
								<div class="afc-radio-container radio-button-item-container" data-value="{$k}" >
									<div class="div left radio-button-own">
										{if ($k == $vacancy['busyTime'])}
											<div class="sprite radio-sprite radio-checked-sprite"><input type="hidden" name="busyTime" value="{$k}" /></div>
											{else}
											<div class="sprite radio-sprite"></div>
											{/if}
									</div>
	...
	<div class="vac-submit">
		<input type="submit" name="save_vac" class="blue-button-input" value="{$m.save}" />
	</div>
	...
	<script>
		var specializations = [
			{foreach from=$profArea name=profArea item=p key=k}
			{
				title: '{$p}',
				id: {$k}
			}
