<?php	
class AdvicesModel extends CRUDModel {
 public $r_id;	
 public $lang_id;
 public $itemTitle;	
 public $content;	
 ...
 $this->forLoggedIn = new ModelSelectField("forLoggedIn", $messages['field_forLoggedIn'], Array(			 '0' => Application::$messages['common_messages']['nno'],				'1' => Application::$messages['common_messages']['yyes'],			), true, true);	
 $this->forLoggedIn->common = true;	
 }	
 public function getSearchUrl() {	
 return 'advice/' . $this->r_id->value;	
 }			
 public static function initialize() {	
 self::$displayFields = Array('image', 'itemTitle', 'date');	
 self::$title = 'Məsləhətlər';		
 self::$iconPath = 'news-icon.png';		
 self::$multiLang = true;	
 self::$searchable = true;	
 self::$viewUrl = 'news';	
 self::$searchSettings = Array(				'title_field' => 'itemTitle',				'content_field' => 'content',			);	
 }		
 public static function getLast($lang, $limit) {	
 $sql = " SELECT * FROM `". self::getTableName() ."` WHERE `lang_id` = '{#1}' LIMIT " . $limit;	
 ...
 $sql = "SELECT `vl1_NewsModel`.*, count(`vl1_NewsCommentsModel`.`newsId`) as `comments_count` 			FROM `vl1_NewsModel`			LEFT JOIN `vl1_NewsCommentsModel` ON `vl1_NewsModel`.`r_id` = `vl1_NewsCommentsModel`.`newsId` 			WHERE `vl1_NewsModel`.`lang_id` = '{#1}' AND `vl1_NewsModel`.`itemTitle` <> '' 			GROUP BY `vl1_NewsModel`.`itemTitle` LIMIT " . ($start * $limit) . "," . $limit;	
 $r = self::fQuery($sql, Array($lang));		
 $c = count($r);	
 for ($i = 0; $i < $c; $i++) {		
 $r[$i]['description'] = Utils::parseWords($r[$i]['content'], 1);	
 ...
 $sql = "SELECT `vl1_NewsModel`.* 			FROM `vl1_NewsModel` 			WHERE 			`vl1_NewsModel`.`lang_id` = '{#1}' 			AND `vl1_NewsModel`.`itemTitle` IS NOT NULL 			AND trim(`vl1_NewsModel`.`itemTitle`) <> '' ";	
 if (!Application::$storage['logged_in']) {		
 $sql .= " AND `forLoggedIn` <> '1' ";		
 }		
 $sql .= " ORDER BY `vl1_NewsModel`.`date` DESC 			LIMIT " . ($start * $limit) . "," . $limit;		
 $r = self::fQuery($sql, Array($lang));		
 $c = count($r);	
 for ($i = 0; $i < $c; $i++) {		
 $r[$i]['description'] = Utils::parseWords($r[$i]['content'], 1);		
 ...
 $sql = "				SELECT `vl1_NewsCommentsModel`.`commentText`,`vl1_NewsCommentsModel`.`date`, `vl1_SiteUsersModel`.* 				FROM `vl1_NewsCommentsModel`				INNER JOIN `vl1_SiteUsersModel` 				ON `vl1_NewsCommentsModel`.`userId` = `vl1_SiteUsersModel`.`id` 				WHERE `vl1_NewsCommentsModel`.`newsId` = '{#1}'			";		
 if (!Application::$storage['logged_in']) {				$sql .= " AND `forLoggedIn` <> '1' ";			}			$comments = self::fQuery($sql, Array($newsId));			return Array(				'news' => $news,				'comments' => $comments,			);
 }
    public static function getAllAutoAdvices($lang, $start, $end) {
        $sql = "Select `itemTitle`,`content`,`date`FROM `".AdvicesModel::getTableName()."`
                    WHERE `lang_id` = '".$lang."' ORDER BY `date` DESC LIMIT ".$start.", ".$end." ";
        $result = self::fQuery($sql);

        return $result;
    }

    public static function getAdvicesCount($lang) {
        $sql = "SELECT COUNT(`id`) FROM `".AdvicesModel::getTableName()."` WHERE `lang_id` = '".$lang."' ";
        $result = self::fQuery($sql);
        return $result;
    }
}
