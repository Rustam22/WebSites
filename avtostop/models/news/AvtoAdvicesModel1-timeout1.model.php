<?php	class AvtoAdvicesModel1 extends CRUDModel {	
	public $r_id;	
	public $lang_id;	
	public $itemTitle;	
	public $content;	
	...
	$this->date->common = true;						$this->forLoggedIn = new ModelSelectField("forLoggedIn", $messages['field_forLoggedIn'], Array(				'0' => Application::$messages['common_messages']['nno'],				'1' => Application::$messages['common_messages']['yyes'],			), true, true);			$this->forLoggedIn->common = true;		}		public function getSearchUrl() {			return 'advice1/' . $this->r_id->value;		}				public static function initialize() {			self::$displayFields = Array('image', 'itemTitle', 'date');			self::$title = 'Sığortalan';			self::$iconPath = 'news-icon.png';			self::$multiLang = true;			self::$searchable = true;			self::$viewUrl = 'news';			self::$searchSettings = Array(				'title_field' => 'itemTitle',				'content_field' => 'content',			);		}				public static function getLast($lang, $limit) {			$sql = " SELECT * FROM `". self::getTableName() ."` WHERE `lang_id` = '{#1}' LIMIT " . $limit;			$r = self::fQuery($sql, Array($lang));			for ($i = 0, $c = count($r); $i < $c; $i++) {				$r[$i]['description'] = Utils::parseWords($r[$i]['content'], 1);			}			return $r;		}				public static function getToView($lang, $rId) {			$sql = " SELECT * FROM `". self::getTableName() ."` WHERE `lang_id` = '{#1}' AND `r_id` = '{#2}' ";			$r = self::fQuery($sql, Array($lang, $rId));			if (count($r) == 1) return $r[0];			else return false;		}		
