        ...
        public static function renderTemplate($templateName, $vars = Array(), $fetch = false) {
            foreach (self::$smarty->getTemplateDir() as $dir) {
				if (is_file($dir . $templateName)) {
					...
					self::$smarty->assign('jsActions', Application::$storage['js_actions']);
					self::$smarty->assign('postData', Application::$storage['post_data']);
					//$csrfKey = Application::getCSRFKey();
					//self::$smarty->assign('csrf_token', $csrfKey);
