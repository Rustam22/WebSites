<?php
	...
	abstract class Application {
		...
		Global $__db;
			...
			// javascripts actions for smarty
			self::$storage['js_actions'] = Array();
			self::$storage['post_data'] = $_POST;
		...
		private static function urlConverter() {
			...
			foreach ($urlsToConvert as $url) {
				if (preg_match('/^' . $url->urlIn->value . '$/im', Controller::$request) || preg_match('/^' . $url->urlIn->value . '\/$/im', Controller::$request)) self::toPattern(self::$urlPatterns['app'][$url->patternIndex->value]);
			}
		}
		...
		private static function serviceModulesUrls() {
			...
			for ($j = 0; $j < $mCount; $j++) {
				...
				for ($i = 0; $i < $urlPatternsCount; $i++) {
					...
					for ($_i = 0; $_i < count($patterns); $_i++) {
						$defVars = isset($url[4]) ? $url[4] : Array();
						if (preg_match_all($patterns[$_i], Controller::$request, $vars, PREG_SET_ORDER)) {
