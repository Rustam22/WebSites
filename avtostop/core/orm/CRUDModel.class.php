    ...
    class CRUDModel extends BaseModel {
		...
		private function update($whereCondition = "", $fields = "") {
			...
			foreach ($sqlData as $k => $v) {
				$sql[] = "`". $k ."` = '". Security::filterSql($v, self::$mysqli) ."'";
			}
		...
		public static function getAllRecordsM($lang = false, $baseField = false, $menu = false) {
			$class = get_called_class();
			$class::initialize();
			$cond = Array();
			$values = Array();
			...
			if ($menu) $cond[] = "`visible` = '1'";
			$classTableName = $class::getTableName();
			$out = ($class::$multiLang) ? $class::fQuery("SELECT * FROM `".$classTableName."` WHERE " . join(" AND ", $cond), $values) : $class::fQuery("SELECT * FROM `".$classTableName."`");
			
			
			$out = self::getDataAsObject($out, self::getClassInfo());
			
			if ($baseField) {
				$newOut = Array();
				foreach ($out as $k => $v) {
					$newOut[$v[$baseField]] = $v;
				}
		...
		public static function getTableName($className = false) {
            ...
            return strtolower(Application::$dbSettings["prefix"] . "_" . $className);
        }
        ...
        public static function getDisplayInfo($start, $count, $order='') {
            //print_r($order);
            Global $__app;
            $query = Array();
            $classInfo = self::getClassInfo();
